...
{ 
    ...
    auto h_cos_phi_cs = m_histContainer->get1DHist("h_cos_phi_cs", 300, -1, 1);
    auto h_cos_phi_ud = m_histContainer->get1DHist("h_cos_phi_ud", 300, -1, 1);
    auto h_cos_phi_cu = m_histContainer->get1DHist("h_cos_phi_cu", 300, -1, 1);  
    auto h_cos_phi_cd = m_histContainer->get1DHist("h_cos_phi_cd", 300, -1, 1); 
    auto h_cos_phi_su = m_histContainer->get1DHist("h_cos_phi_su", 300, -1, 1);  
    auto h_cos_phi_sd = m_histContainer->get1DHist("h_cos_phi_sd", 300, -1, 1); 
    ...
    for(int i = 0; i < nEntries; i++)
    {
       ...
       if (nPrinted < maxPrint) {
        ...
        // Calculates the total momentum for W1 and W2
        h_W1_p->Fill(W1_option1.P());
        h_W2_p->Fill(W2_option1.P());

        // Angles between all the jets is W1_->cs and W2-->ud decay channels
        CosPhi_Angle(cTag_Jet, lTag_Jet_0, h_cos_phi_cs);
        CosPhi_Angle(lTag_Jet_1, lTag_Jet_2, h_cos_phi_ud);
        CosPhi_Angle(cTag_Jet, lTag_Jet_1, h_cos_phi_cu);
        CosPhi_Angle(cTag_Jet, lTag_Jet_2, h_cos_phi_cd);
        CosPhi_Angle(lTag_Jet_0, lTag_Jet_1, h_cos_phi_su);
        CosPhi_Angle(lTag_Jet_0, lTag_Jet_2, h_cos_phi_sd);
